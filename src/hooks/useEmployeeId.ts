...
import { generateEmployeeId, countEmployeesByDepartment } from '../utils/employeeId';
 ...
 */
export function useEmployeeId(departmentName: string | undefined): UseEmployeeIdReturn {
  ...
  useEffect(() => {
    ...
    const generateId = async () => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : 'Failed to generate employee ID');
        setEmployeeId('');
      } finally {
        setIsLoading(false);
      }
